...
//    int destination;
//    
//    map<int, int>nodeTable;     //输入的点的编号->从0开始的编号
//    map<Link*, int>edgeTable;
//    Link *edgeIndexTable[MAXM * 2];  //把链路从0开始编号
//    
//    int flowSrc[MAXN], srcCnt;  // 视频流源点的index，个数
//    int flowDst;
//    
//    int src[MAXM * 2]; //edge index -> edge src index
//    int dst[MAXM * 2];
//    int restCap[MAXM * 2];  //edge index -> rest capacity
//    int bgCap[MAXM * 2];    //edge index -> bg flow
//    
...
//              const list<int>&flowSrc_, const int flowDst_)
//    {
//        nodeTable.clear();
//        edgeTable.clear();
//        nodeNum = 0;
...
//        srcCnt = 0;
//        for(auto x : flowSrc_)
//            flowSrc[srcCnt++] = nodeTable[x];
//        flowDst = nodeTable[flowDst_];
//    }
...
//        {
//            int rest = max(restCap[i] + min(padding, bgCap[i]) - threshold, 0);
//            addLink(src[i], dst[i], rest / bandWidth);   //单向边
//        }
//        int source = nodeNum + 1;
//        for(int i=0;i<srcCnt;i++)
//            addLink(source, flowSrc[i], 1);
//        int flow = SAP(source, flowDst);
//        if(flow == srcCnt)
//        {
//            linkLimit.clear();
//            for(int i=0;i<edgeNum;i++)
...
//                   const map<Link*, int>&linkCost, const map<Link*, int>&linkBg,
//                   const list<int>&flowSrc, int flowDst, int bandWidth, int threshold,
//                   list<list<Link*>>&flowPath, map<Link*, int>&linkLimit)
////如果不需要限速 那么linkLimit为空
//{
//    int maxBandWidth = 0;
//    for(auto x : linkBg)
//        maxBandWidth = max(maxBandWidth, x.second); //所有链路中的最大背景流大小
//    
...
//int main()
//{
//    freopen("/Users/ningjieqian/Downloads/SDN比赛/data/1.txt", "r", stdin);
//    freopen("/Users/ningjieqian/Downloads/SDN比赛/data/newout/1.out", "w", stdout);
//    int n,m;
//    scanf("%d%d",&n,&m);
//    
...
//    rearrangeFlow(links, n,linkCost,linkBg,flowSrc,flowDst,bandWidth,0,flowPath, linkLimit); //0-threshold
